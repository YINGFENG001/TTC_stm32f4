...
- 传入带 `%` 的整数时按实测物理开度百分比解析，范围 `0%~100%`，`100%` 等同 `700`，对应最大张开约 `72mm`；`0%` 等同 `2048`。
- `status` 和夹爪动作返回中的位置统一显示为 `openPos/Pct = 700 (100.0%)`，其中百分比由 `Gripper_PositionToOpenPercentX10()` 按实测表插值得出。

当前物理开度标定表在 `User/gripper/bsp_gripper.c` 中维护：
...
其中 `100.0%` 为实测最大开口约 `72mm`，其余百分比为相对该最大物理开口的开度。
