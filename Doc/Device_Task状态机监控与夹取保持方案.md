...
当前快速故障检测固定使用 `200ms`，不跟随 `monitor [intervalMs]` 改变。执行 `monitor` 时会打印当前 `interval`、`fast_check` 和 `fault_hits`。
...
`openPercentage` 为可选参数：

- 不传时，从当前位置开始精细闭合。
- 传入时，先运动到指定开口百分比，`0=闭合`，`100=张开`。
...
1. 如传入 `openPercentage`，夹爪快速运动到该开口位置。
2. 快速运动过程中，若负载达到目标负载的 `30%`，提前结束快速运动。
...
电机无进展检测由 `Device_MonitorTask()` 执行，周期为 `DEVICE_FAST_CHECK_INTERVAL_MS`。
...
- 电机处于 `running` 状态。
- 原始脉冲位置连续 `5` 个快速检测周期不变。

满足条件后，执行 `Stepper_Stop(i)`，设备状态置为 `DEV_ERROR`，并上报：

```text
@fault dev=mtor1 event=stall pos=12.3 target=50.0 action=stop
```
